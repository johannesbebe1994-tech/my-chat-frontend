...
const GREETINGS = ['Welcome back.', 'Ready when you are.', 'What\'s on your mind?', 'Let\'s talk.', 'A fresh start.', 'The night is yours.', 'Say something.']
function App() {
 ...
 const [coverImg, setCoverImg] = useState(localStorage.getItem('cover_img') || '')
 const [copiedIdx, setCopiedIdx] = useState(null)
 const [stats, setStats] = useState({ sessions: 0, messages: 0 })
 const messagesEndRef = useRef(null)
 const messagesContainerRef = useRef(null)
 const todayGreeting = GREETINGS[new Date().getDate() % GREETINGS.length]
 useEffect(() => { loadSessions(); loadSettings(); loadStats(); document.documentElement.setAttribute('data-theme', theme) }, [])
 useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }) }, [messages])
 useEffect(() => { document.documentElement.setAttribute('data-theme', theme); localStorage.setItem('theme', theme) }, [theme])
 useEffect(() => { const c = messagesContainerRef.current; if (!c) return; const h = () => setShowScrollTop(c.scrollTop > 300); c.addEventListener('scroll', h); return () => c.removeEventListener('scroll', h) }, [currentSession])
 async function loadSessions() { const r = await fetch(`${API}/sessions`); setSessions(await r.json()) }
 async function loadSettings() { const r = await fetch(`${API}/settings`); setSettings(await r.json()) }
 async function loadStats() { try { const r = await fetch(`${API}/stats`); setStats(await r.json()) } catch {} }
 ...
 function goHome() { setCurrentSession(null); setMessages([]); loadSessions(); loadStats() }
 ...
 let aiReply = '', aiThinking = '', buf = ''
 setMessages(p => [...p, { role: 'assistant', content: '', thinking: '', created_at: new Date().toISOString() }])
 while (true) {
 ...
 if (data.reasoning) { aiThinking += data.reasoning; setMessages(p => { const u = [...p]; u[u.length - 1] = { ...u[u.length - 1], thinking: aiThinking }; return u }) }
 if (data.content) { aiReply += data.content; setMessages(p => { const u = [...p]; u[u.length - 1] = { ...u[u.length - 1], content: aiReply }; return u }) }
 if (data.done && data.title) { setCurrentSession(p => ({ ...p, name: data.title })); setSessions(p => p.map(s => s.id === sessionId ? { ...s, name: data.title } : s)) }
 if (data.error) { setMessages(p => { const u = [...p]; u[u.length - 1] = { ...u[u.length - 1], content: `出错了: ${data.error}` }; return u }) }
 } catch {}
 }
 }
 if (!aiReply) setMessages(p => { const u = [...p]; u[u.length - 1] = { ...u[u.length - 1], content: '未收到回复，请重试' }; return u })
 } catch { setMessages(p => [...p.filter(m => m.content || m.role === 'user'), { role: 'assistant', content: '发送失败，请重试', created_at: new Date().toISOString() }]) }
 finally { setLoading(false) }
 }
 async function send() { if (!input.trim() || !currentSession || loading) return; const t = input.trim(); setInput(''); setMessages(p => [...p, { role: 'user', content: t, created_at: new Date().toISOString() }]); await streamChat(currentSession.id, t) }
 async function retry() { if (loading || messages.length < 2 || !currentSession) return; const lu = [...messages].reverse().find(m => m.role === 'user'); if (!lu) return; setMessages(p => p.slice(0, p.length - 1)); await streamChat(currentSession.id, lu.content, true) }
 ...
 async function submitEdit() { if (!editText.trim() || loading || !currentSession) return; const t = editText.trim(), kc = editingIdx; setEditingIdx(null); setEditText(''); setMessages(p => [...p.slice(0, kc), { role: 'user', content: t, created_at: new Date().toISOString() }]); await fetch(`${API}/messages/truncate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: currentSession.id, keep_count: kc }) }); await streamChat(currentSession.id, t) }
 ...
 async function copyMessage(c, i) { try { await navigator.clipboard.writeText(c); setCopiedIdx(i); setTimeout(() => setCopiedIdx(null), 1500) } catch {} }
 function exportChat(fmt) { if (!currentSession || !messages.length) return; let c, f; if (fmt === 'json') { c = JSON.stringify({ session: currentSession.name, messages: messages.map(m => ({ role: m.role, content: m.content, time: m.created_at })) }, null, 2); f = `${currentSession.name}.json` } else { c = messages.map(m => `[${m.role === 'user' ? '我' : 'AI'}]\n${m.content}`).join('\n\n---\n\n'); f = `${currentSession.name}.txt` }; const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([c])); a.download = f; a.click() }
 function handleImageUpload(e, key, setter, maxSize) { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (ev) => { const img = new Image(); img.onload = () => { const canvas = document.createElement('canvas'); let w = img.width, h = img.height; if (w > maxSize || h > maxSize) { if (w > h) { h = (h / w) * maxSize; w = maxSize } else { w = (w / h) * maxSize; h = maxSize } }; canvas.width = w; canvas.height = h; canvas.getContext('2d').drawImage(img, 0, 0, w, h); const d = canvas.toDataURL('image/png', 0.9); setter(d); localStorage.setItem(key, d) }; img.src = ev.target.result }; reader.readAsDataURL(file); e.target.value = '' }
 ...
 <div className="search-box"><input className="search-input" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') searchMessages(searchQuery) }} placeholder="搜索聊天记录..." />{searchQuery && <button className="search-clear" onClick={clearSearch}>✕</button>}</div>
 {isSearching ? <div className="session-list">{searchResults.length === 0 ? <div className="no-results">没有找到</div> : searchResults.map((r, i) => <div key={i} className="search-result-item" onClick={() => { selectSession({ id: r.session_id, name: r.session_name }); clearSearch() }}><div className="search-result-title">{r.session_name}</div><div className="search-result-preview">{r.content.substring(0, 50)}</div></div>)}</div> : <div className="session-list">{sessions.map(s => <div key={s.id} className="session-item" onClick={() => selectSession(s)}><span className="session-name">{s.name}</span><button className="delete-btn" onClick={e => deleteSession(e, s.id)}>×</button></div>)}</div>}
 ...
 <span className="cover-title">HOME</span>
 ...
 <div className="bento-pic" style={coverImg ? { backgroundImage: `url(${coverImg})` } : {}}>
 {!coverImg && <div className="bento-pic-placeholder">TAP TO SET</div>}
 <label className="bento-pic-upload"><input type="file" accept="image/*" hidden onChange={e => handleImageUpload(e, 'cover_img', setCoverImg, 600)} /></label>
 </div>
 <div className="bento-greeting">
 <div className="bento-label">GREETING</div>
 <div className="bento-greeting-text">{todayGreeting}</div>
 </div>
 <div className="bento-stat1">
 <div className="bento-stat-num">{stats.messages}</div>
 <div className="bento-label">MESSAGES</div>
 </div>
 <div className="bento-stat2">
 <div className="bento-stat-num">{stats.sessions}</div>
 <div className="bento-label">CHATS</div>
 </div>
 <div className="bento-newchat" onClick={createSession}>
 <div className="bento-newchat-plus">+</div>
 <div className="bento-label">NEW CHAT</div>
 </div>
 <div className="bento-recent">
 <div className="bento-label">RECENT</div>
 <div className="bento-recent-list">
 {sessions.slice(0, 4).map(s => <div key={s.id} className="bento-recent-item" onClick={() => selectSession(s)}>{s.name}</div>)}
 {sessions.length === 0 && <div className="bento-recent-empty">还没有对话</div>}
 </div>
 </div>
 </div>
 </div>
 {showSettings && settings && renderSettings()}
 </div>
 )
 // ========== 聊天界⾯ ==========
 function renderSettings() {
 return (<><div className="overlay" onClick={() => setShowSettings(false)} /><div className="settings-panel">
 <h3>设置</h3>
 ...
 <div className="toggle-row"><label>叠加提示词</label><button className={`toggle-btn ${settings.overlay_enabled ? 'on' : ''}`} onClick={() => setSettings({ ...settings, overlay_enabled: !settings.overlay_enabled })}>{settings.overlay_enabled ? '开' : '关'}</button></div>
 {settings.overlay_enabled && <textarea className="prompt-input" value={settings.overlay_prompt || ''} onChange={e => setSettings({ ...settings, overlay_prompt: e.target.value })} rows={4} placeholder="场景/模式提示词..." />}
 <div className="toggle-row"><label>时间感知</label><button className={`toggle-btn ${settings.time_aware ? 'on' : ''}`} onClick={() => setSettings({ ...settings, time_aware: !settings.time_aware })}>{settings.time_aware ? '开' : '关'}</button></div>
 ...
 <label>导出当前对话</label>
 <div className="export-row"><button className="export-btn" onClick={() => exportChat('txt')}>导出 TXT</button><button className="export-btn" onClick={() => exportChat('json')}>导出 JSON</button></div>
 <hr className="settings-divider" />
 <label>外观</label>
 ...
 <div className="appearance-item"><span>封⾯图⽚</span><div className="bg-preview">{coverImg ? <img src={coverImg} alt="" /> : '⽆'}</div><label className="upload-label">上传<input type="file" accept="image/*" hidden onChange={e => handleImageUpload(e, 'cover_img', setCoverImg, 600)} /></label>{coverImg && <button className="clear-btn" onClick={() => clearImage('cover_img', setCoverImg)}>清除</button>}</div>
 ...
 <div className="sidebar-header"><span>对话列表</span><button className="new-chat-btn" onClick={createSession}>+</button></div>
 <div className="search-box"><input className="search-input" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') searchMessages(searchQuery) }} placeholder="搜索聊天记录..." />{searchQuery && <button className="search-clear" onClick={clearSearch}>✕</button>}</div>
 {isSearching ? <div className="session-list">{searchResults.length === 0 ? <div className="no-results">没有找到</div> : searchResults.map((r, i) => <div key={i} className="search-result-item" onClick={() => { selectSession({ id: r.session_id, name: r.session_name }); clearSearch() }}><div className="search-result-title">{r.session_name}</div><div className="search-result-preview">{r.content.substring(0, 50)}</div></div>)}</div> : <div className="session-list">{sessions.map(s => <div key={s.id} className={`session-item ${currentSession?.id === s.id ? 'active' : ''}`} onClick={() => selectSession(s)}><span className="session-name">{s.name}</span><button className="delete-btn" onClick={e => deleteSession(e, s.id)}>×</button></div>)}</div>}
 <div className="sidebar-footer"><button className="settings-btn" onClick={() => { setShowSettings(true); setSidebarOpen(false) }}>⚙ 设置</button></div>
 </div>
 <div className="chat-container">
 <div className="chat-header">
 <button className="menu-btn" onClick={() => setSidebarOpen(!sidebarOpen)}>☰</button>
 <button className="home-btn" onClick={goHome}>首页</button>
 {editingTitle ? (
 ...
 <div className="edit-area"><textarea className="edit-input" value={editText} onChange={e => setEditText(e.target.value)} rows={3} /><div className="edit-actions"><button className="cancel-edit-btn" onClick={cancelEdit}>取消</button><button className="save-edit-btn" onClick={submitEdit}>发送</button></div></div>
 ) : (<>
 ...
 <div className="message assistant"><div className="message-header"><div className="avatar">{aiAvatar ? <img src={aiAvatar} alt="" /> : ' '}</div></div><div className="bubble assistant thinking">思考中...</div></div>
 )}
